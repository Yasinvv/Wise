...
SRC = source/main.cpp
SHADER_FILE_PATH = data/shaders/slang.spv
CPPFLAGS = -std=c++23 \
					 -DSHADER_PATH="\"$(SHADER_FILE_PATH)\""\
					 -DVULKAN_HPP_NO_STRUCT_CONSTRUCTORS
SDLTest: $(SRC)
	g++ $(CPPFLAGS) -o prog $(SRC) \
...
shader:
	shader.slang -target spirv -profile spirv_1_4 -emit-spirv-directly -fvk-use-entrypoint-name -entry vertMain -entry fragMain -o slang.spv
